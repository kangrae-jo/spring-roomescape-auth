...
| 용어    | 설명                   |
|-------|----------------------|
| admin | 방탈출 매장을 관리할 수 있는 권한  |
| guest | 방탈출에 로그인 한 사용자 기본 권한 |

### 매장

| 용어      | 설명                                |
|---------|-----------------------------------|
| store   | 방탈출 매장. 매장별로 테마 종류와 시간 슬롯은 같다고 가정 |
| manager | 특정 매장의 예약을 관리할 수 있는 사용자           |
...
- [ ] 매장 매니저 식별
    - [x] 로그인한 사용자가 매장 매니저인지 확인할 수 있어야 한다.
    - [ ] 매장 매니저가 어떤 매장을 관리하는지 확인할 수 있어야 한다.

- [ ] 예약 접근 제한
    - [x] 매장 매니저는 자기 매장의 예약만 조회, 변경, 삭제할 수 있다.
    - [x] 다른 매장의 예약에 접근하려는 요청은 거부한다.

- [x] 실패 처리
    - [x] 로그인하지 않은 요청은 인증 실패로 처리한다.
    - [x] 로그인했지만 권한이 없는 요청은 인가 실패로 처리한다.
    - [x] 인증 실패와 인가 실패를 같은 문제로 뭉개지 않는다.

- [ ] 구현 조건
    - [ ] 로그인한 사용자가 어떤 매장에 속해 있는지 확인할 수 있어야 한다.
    - [ ] 접근하려는 예약이 어떤 매장에 속해 있는지 확인할 수 있어야 한다.
    - [ ] 두 정보를 비교해 요청을 허용하거나 거부한다.
    - [x] 인가 판단 로직이 컨트롤러에 과도하게 흩어지지 않도록 한다.
