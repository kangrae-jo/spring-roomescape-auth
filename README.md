...
- [x] 구현 조건
    - [x] 세션과 토큰 중 하나를 선택하거나, 두 방식을 조합할 수 있다.
    - [x] 선택한 방식 외의 다른 후보도 검토한다.
    - [x] 인증 정보를 전달하는 위치를 명확히 정한다.
    - [x] 컨트롤러는 웹인지 모바일인지에 따라 인증 로직이 과도하게 갈라지지 않도록 한다.
